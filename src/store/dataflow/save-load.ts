...
import { showSystemMessage, clearSystemMessage } from '@/store/message';
import { axiosPost, systemMessageErrorHandler, errorMessage } from '@/common/util';
    ...
    return new Promise((resolve, reject) => {
      axiosPost<string>('/diagram/save-as', {
        ...
        diagramName,
          ...
          router.replace(`/diagram/${filename}`);
    ...
    return new Promise((resolve, reject) => {
      axiosPost<DiagramSave>('/diagram/load', { filename })
        .then(res => {
          ...
          showSystemMessage(`Diagram loaded: ${res.data.diagramName}`, 'success');
        ...
        .catch(err => {
          if (err.response && err.response.data === 'no access') {
            showSystemMessage('login to view this diagram', 'error');
